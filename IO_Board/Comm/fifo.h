 ...
 * @note This is a really heavy library but can solve almost every fifo problem, if that's a problem, use C++ templates instead :-)
 */
 ...
 */
#define FIFO_ALLOW_MALLOC   false
...
typedef struct{
    ...
    volatile FIFO_INDEX_TYPE read_idx;               /*!< read index for fifo read access, offset from pFifo in bytes */
    volatile FIFO_INDEX_TYPE write_idx;              /*!< write index for fifo write access, offset from pFifo in bytes */
    volatile void *pFifo;                            /*!< pointer to the first adress of the fifo memory */
}fifo_handle_t;
 ...
 */
int8_t fifo_init(fifo_handle_t *pHandle, void *pFifo, FIFO_INDEX_TYPE size_fifo, SIZE_FIFO_BASE_TYPE basetype_size);
 ...
 * @param pHandle pointer to the Fifo handle
 */
void fifo_deinit_free(fifo_handle_t *pHandle);
 ...
 */
bool fifo_hasElementsLeft(fifo_handle_t *pHandle);
 ...
 */
bool fifo_hasSpaceLeft(fifo_handle_t *pHandle);

/**
 * @brief flushes a FIFO
 * @return FIFO_NO_ERROR    Everything worked
 */
fifoerror_t fifo_flush(fifo_handle_t *pHandle);
