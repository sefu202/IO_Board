 ...
 * @brief Implementation of the fifo library
 * @version 1.0
 * @date 2022-01-23
 * 
...
/**
 * @brief Volatile safe memcpy
 * @param dest where to write to
 * @param src where to read from
 * @param size size of the memory to copy
 * @return pointer to dest
 */
static volatile void *memcpy_v(volatile void* dest, const volatile void* src, uint8_t size){
	while (size--){
		((volatile uint8_t*)dest)[size] = ((volatile uint8_t*)src)[size];
	}
	return dest;
}
...
fifoerror_t fifo_put(fifo_handle_t *pHandle, const void *pData)
{
    ...
    {
        // *** Write to the fifo ***
        memcpy_v(((volatile uint8_t *)(pHandle->pFifo) + (pHandle->write_idx = idx_temp)), pData, pHandle->basetype_size);
    }
...
fifoerror_t fifo_get(fifo_handle_t *pHandle, void *pData)
{
    ...
    if (write_idx != pHandle->read_idx)  // no data in fifo
    {
        ...
        // *** Copty the data ***
        memcpy_v(pData, ((volatile uint8_t *)(pHandle->pFifo)) + (pHandle->read_idx = idx_temp), pHandle->basetype_size);  
    }
    else
    {
        ret = FIFO_EMPTY;
